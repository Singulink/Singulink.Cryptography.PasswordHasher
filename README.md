...
### Turning normalization on or off

`PasswordHasherOptions` can be used to specify additional options like normalization settings, salt size, etc.
...
// GetMasterKey1() should get the key from somewhere other than the database (secure storage, config file, hard-coded, etc).
...
```cs
// Create Argon2id algorithm with degree of paralellism: 4, memory: 512MB, and 256-bit output.
